...
import org.matrix.dma.gchat.extlib.bidiweb.webchannel.client.support.Support.RequestErrorCode;
import org.matrix.dma.gchat.extlib.bidiweb.webchannel.client.support.Support.RequestReadyState;
import org.matrix.dma.gchat.extlib.bidiweb.webchannel.client.support.Support.UriBuilder;
...
class BasicWebChannelSupportHttpRequest
    extends org.matrix.dma.gchat.extlib.bidiweb.webchannel.client.support.Support.HttpRequest {
  ...
  private void sendSynchronously(
    ...
    try {
      Log.d("bidiweb",
              "Sending HTTP " + verb + " request: "+ postData+" to url: "+uri+" with headers: "+headers+" ("+this+")");
  ...
  private void readResponseHeaders(HttpResponse response) {
    synchronized (lock) {
      ...
      if (response.isSuccessStatusCode()) {
        ...
        Log.d("bidiweb", "HTTP channel request failed with status: " + status);
      }
    }
  }
  ...
  private void readResponseBody(HttpResponse response) throws IOException {
    ...
    if (content == null) {
      Log.d("bidiweb", "No content in channel response");
      return;
    }
    String encoding =
  ...
  private void processChunk(CharBuffer chunk) {
    Log.d("bidiweb", "Read " + chunk.length() + " chars from the channel: " + chunk);
    synchronized (lock) {
  ...
  private void finishReadingResponse() {
    Log.d("bidiweb", "Finished reading channel response");
    synchronized (lock) {
  ...
  private void processRequestError(IOException exception) {
    Log.d("bidiweb", "HTTP channel request failed: " + exception.toString());
    synchronized (lock) {
      if (aborted) {
